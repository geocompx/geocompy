...
The **geopandas** package provides the `.simplify` method, which uses the GEOS implementation of the Douglas-Peucker algorithm to reduce the vertex count. `.simplify` uses the `tolerance` to control the level of generalization in map units (see [@douglas_algorithms_1973] for details). 
...
Figure @fig-simplify-polygons demonstrates the two simplification methods applied to `us_states9311`.
...
us_states9311.plot(ax=axes[0], color='lightgrey', edgecolor='black')
