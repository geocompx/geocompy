...
Keep in mind that `x` and `y` are interchangeable in all predicates except for `.difference`, where:
...
To create the latter, let's take `x` and combine it with itself translated (@sec-affine-transformations) to a distance of `1` or `2` units "upwards" on the y-axis:
...
Similarly, a `'Polygon'` can be created using function `shapely.Polygon`, which accepts a sequence of point coordinates. In principle, the last coordinate must be equal to the first, in order to form a closed shape. However, `shapely.Polygon` is able to complete the last coordinate automatically. Therefore (@fig-type-transform-polygon):
