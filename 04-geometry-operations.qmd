...
Also see the ["Set operations with overlay"](https://geopandas.org/en/stable/docs/user_guide/set_operations.html) article in the **geopandas** documentation for examples of `.overlay`.
...
Behind the scenes, `.dissolve` combines the geometries and dissolve the boundaries between them using the `.union_all` method per group.
...
To dissolve two (or more) groups of a `GeoDataFrame` into one geometry, we can either (a) use a combined condition or  (b) concatenate the two separate subsets and then dissove using `.union_all`.
...
The `.geoms` property, mentioned in @sec-geometries, give access to the indivudual parts that comprise a multi-part geometry, as an iterable object similar to a `list`; it is one of the **shapely** access methods to internal parts of a geometry.
