...
The result is a `GeoDataFrame` with numerous properties. The following expression plots the geometries with the `name` property in the tooltips (@fig-ox-features):
...
To import and convert it to a `GeoDataFrame`, we can apply the `shapely.from_wkt` function (@sec-geometries) on WKT strings, to convert them into `shapely` geometries:
