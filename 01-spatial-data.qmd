...
-   **rasterio** is a spatial-oriented package, focused on "simple" raster formats (such as GeoTIFF), representing a raster using a combination of a **numpy** array, and a metadata object (`dict`) specifying the spatial reference of the array

Another raster-related package worth mentioning is **xarray**.
It is a general-purpose package for working with labeled arrays, thus advantageous for processing "complex" raster formats (such as NetCDF), representing a raster using its own native classes, namely `xarray.Dataset` and `xarray.DataArray`.
...
-   Vector datasets and methods dominate the social sciences because human settlements and and processes (e.g., transport infrastructure) tend to have discrete borders<!--jn: is that true?-->
...
There is more to CRSs, as described in @sec-coordinate-reference-systems and @sec-reproj-geo-data but, for the purposes of this section, it is sufficient to know that coordinates consist of two numbers representing the distance from an origin, usually in $x$ then $y$ dimensions.
...
<!--jn: type(gdf) is not printed as an output-->
<!-- md: fixed -->
...
Alternatively, we can get a more interactive plot by using `.explore` (@fig-gdf-explore).
...
In case we want the latter, we can create a copy of the `GeoDataFrame` and then "overwrite" its geometry (or, we can overwrite the geometries directly in case we do not need the original ones, as in `gdf.geometry=gdf.centroid`).
...
@fig-switch-to-centroids and @fig-switch-to-polygons shows interactive maps of the `gdf` layer with the `'centroids'` and `'polygons'` geometry columns activated, respectively.
...
However, the Simple Features standard allows multiple geometries to exist within a single feature, using "multi" versions of each geometry type, as illustrated in @fig-multipoint, @fig-multilinestring, and @fig-multipolygon.
...
A `'LineString'` geometry can be created based on a list of coordinate tuples (@fig-linestring).
...
A `'MultiPoint'` geometry is also created from a list of coordinate tuples (@fig-multipoint), where each element represent a single point.
...
A `'MultiPolygon'` geometry (@fig-multipolygon) is created from a `list` of `Polygon` geometries.
