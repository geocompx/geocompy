...
This chapter introduces key Python packages and data structures for working with the two major types of spatial data: **shapely** and **geopandas** for working with geograpic vector data, and **rasterio** for working with rasters.
...
-   **rasterio** represents "simple" raster datasets with a combination of a **numpy** array, and a metadata object (`dict`) providing geographic metadata such as the coordinate system

**xarray** is an alternative to **rasterio** for working with a range of raster datasets that we do not cover in detail in this book.
**xarray** supports complex raster formats such as NetCDF and uses native `xarray.Dataset` and `xarray.DataArray` classes to represent raster data.
