...
# print("path exists") # directory exists
    ...
    r = requests.get(
        "https://github.com/geocompx/geocompy/releases/download/0.1/data.zip",
        timeout=10,
    )
    with zipfile.ZipFile(io.BytesIO(r.content)) as zip_buffer:
        zip_buffer.extractall(".")
...
# print("path exists") # directory exists
    ...
    with zipfile.ZipFile(io.BytesIO(r.content)) as zip_buffer:
        zip_buffer.extractall(".")
    shutil.copytree("geocompy-main/data", "data", dirs_exist_ok=True)
...
# gdf.hvplot(
# title="Hello world", geo=True, hover_cols=["name_long"], legend=False
# ).opts(
# bgcolor="lightgray", active_tools=["wheel_zoom"]
# )
# This way, we can also add background tiles:
# gdf.hvplot(
# tiles="OSM", alpha=0.5, geo=True, title="Hello world", hover_cols=["name_long"], legend=False
# ).opts(
...
#| fig-cap: Switch to the `'bbox'` geometry column in the `world` layer, and plot it
...
#| fig-cap: Switch to the `"polygons"` geometry column in the `world` layer, and plot it
...
#| fig-cap: Example `elev` raster plot, a minimal continuous raster created from scratch
...
#| fig-cap: Example `grain` raster plot, a minimal categorical example created from scratch
