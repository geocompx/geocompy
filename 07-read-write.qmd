...
The GEOSS portal[^geoss_portal] and the Copernicus Open Access Hub[^copernicus], for example, contain many raster datasets with global coverage.
...
Note that @fig-ne-counties x-axis spans the entire range of longitues, between `-180` and `180`, since the Aleutian Islands county (which is small and difficult to see on the map) crosses the International Date Line.
