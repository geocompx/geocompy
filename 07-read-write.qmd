...
URL = (
    ...
    "http//www.naturalearthdata.com/download/10m/cultural/ne_10m_airports.zip"
)
...
# Extract
with zipfile.ZipFile(FILENAME, "r") as zip_buffer:
    zip_buffer.extractall("output")
```
...
```{python}
placemarks = gpd.read_file(URL, layer="Placemarks")
placemarks
```
...
#| eval: false
src = rasterio.open(
    "/vsicurl_streaming/https://zenodo.org/record/5774954/files/"
...
```{python}
#| eval: false
this_window = rasterio.windows.from_bounds(
    left=XMIN, bottom=YMIN, right=XMAX, top=YMAX, transform=src.transform
)
this_window
```
...
Now we can read the partial array, according to the specified window `this_window`, by passing it to the `.read` method.

```{python}
#| eval: false
r = src.read(1, window=this_window)
r
```
