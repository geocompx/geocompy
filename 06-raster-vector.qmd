...
Second, we "cast" the polygon into a `"MultiLineString"` geometry, using the `.boundary` property that `shapely` geometries have: 

```{python}
california_borders = california['geometry'].iloc[0].boundary
california_borders
```
...
```{python}
bounds = california_borders.bounds
res = 0.5
transform = rasterio.transform.from_origin(
    west=bounds[0], 
    north=bounds[3], 
