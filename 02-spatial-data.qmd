...
The third class is defined in package **shapely**, which deals with individual geometries, and is a main dependency of the **geopandas** package.
...
Before we begin, we need to import the **geopandas** package, conventionally as `gpd`, as well as `shapely`:
...
```{python}
multipolygon = shapely.geometry.MultiPolygon([Polygon([(1,5), (2,2), (4,1), (4,4), (1,5)]), 
...
```{python}
multipoint = shapely.geometry.GeometryCollection([MultiPoint([(5,2), (1,3), (3,4), (3,2)]),
                                 MultiLineString([[(1,5), (4,4), (4,1), (2,2), (3,2)], [(1,2), (2,4)]])])
multipoint
```
...
These are suitable because the Earth is compressed: the equatorial radius is around 11.5 km longer than the polar radius (Maling 1992...).
...
Projected CRSs are based on Cartesian coordinates on an implicitly flat surface (right panel of Figure ...). 
