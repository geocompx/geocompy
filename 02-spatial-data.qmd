...
Area and length can be calculated for any of the data structures discussed above, either a `shapely` geometry, in which case the returned value is a number:
...
To obtain true length and area measurements, the geometries first need to be transformed to a projected CRS (see @sec-reprojecting-vector-geometries). For example, the area of egypt can be calculated in the UTM zone 37N CRS (`crs=32637`). The result is in the CRS units, namely $m^2$:

```{python}
gdf[gdf['name_long'] == 'Egypt'].to_crs(32637).area
```
