...
There are three main classes for working with geographic vector data in Python.
These are hierarchical, meaning the highest level (`GeoDataFrame`) class is composed of lower level 'geometry column' and individual geometry components.
This section introduces them in order, starting with the highest level class.
For many applications high level data frame-like objects are all you need need to know about.
