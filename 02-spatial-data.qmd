...
The two following commands therefore return exactly the same result, a `GeoSeries` with country centroids (note: only the geometry is returned, not the attribute data):
