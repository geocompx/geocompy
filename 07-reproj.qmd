...
The above example, where we automatically calculate the destination grid, is appropriate when there are no specific requirements for the raster spatial properties, but rather we just wish to preserve the raster values as much as possible (this the extent and resolution are as similar as possible to the source). 
