...
```{python}
world.iloc[0:3, :]                ## subset rows by position
```

```{python}
world.iloc[:, 0:3]                ## subset columns by position
```

```{python}
world.iloc[0:3, 0:3]              ## subset rows and columns by position
```

```{python}
world[["name_long", "geometry"]]  ## columns by name
```

```{python}
x = np.array([1, 1, 0, 0, 0, 0, 0, 1, 1, 0, 0], dtype=bool)
world.iloc[:, x]                  ## by logical indices
```
