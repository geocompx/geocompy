...
To change all column names at once, by position, we assign a `list` of the "new" column names into the `.columns` property. The `list` must be of the same length as the number of columns (i.e., `world.shape[1]`). This is illustrated below, which outputs the same `world2` object, but with very short names:
...
1. Converting from `GeoDataFrame` into a `DataFrame`
