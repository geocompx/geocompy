...
In this case, `world_many_layers.gpkg` has two "layers", `world_many_layers` (same as the file name, when `layer` is unspecified) and `world2`. Incidentally, the contends of the two layers is identical, but this doesn't have to be so. Each layer from such a file can be imported separately, as in: 
