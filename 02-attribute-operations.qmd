...
TIF_URL = "https://github.com/geocompx/geocompy/releases/download/0.1/landsat.tif"
if not os.path.exists(FILE_PATH):
    print("Attempting to get the data")
    urlretrieve(TIF_URL, FILE_PATH)
```
<!--wd: Using the UPPER_CASE constant naming convention.-->
```{python}
...
```{python}
world[world["continent"] == "Asia"].loc[:, ["name_long", "continent"]].iloc[0:5]
```
...
idx_continent = (world["continent"] == "North America") | (world["continent"] == "South America")
world[idx_continent].loc[:, ["name_long", "continent"]]
```
...
```{python}
list_continent = ["North America", "South America"]
world[world["continent"].isin(list_continent)].loc[:, ["name_long", "continent"]]
```
...
#| fig-cap: "Continent's properties calculated using different spatial aggregation functions"
#| fig-subcap: 
#| - Summed population
#| - Summed area
#| - Count of countries
...
```{python}
new_names = "a,b,c,d,e,f,g,h,geom,i,j,k,l".split(",")
