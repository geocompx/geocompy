...
For the coastline, we will dissolve the New Zealand administrative division polygon layer and "extract" the boundary as a `'MultiLineString'` geometry. Note that `.dissolve(by=None)` (@sec-vector-attribute-aggregation) calls `.unary_union` on all geometries (i.e., aggregates everything into one group), which is what we want to do here.

```{python}
coastline = nz.dissolve().to_crs(src_nz_elev.crs).boundary
coastline
```
