...
Furthermore, the `rasterio.features.rasterize` function requires the input vector shapes in the form of an iterable object of `geometry,value` pairs, where:
...
Furthermore, we define how to deat with multiple values burned into the same pixel, using the `merge_alg` parameter.
...
How the `rasterio.features.rasterize` function works will be made clear in the next examples.
