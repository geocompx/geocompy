...
However, spoecifically for the "multi-part to single part" type transformation scenarios, there is also a method called `.explode`, which can convert an entire multi-part `GeoDataFrame` to a single-part one. The advantage is that the original attributes (such as `id`) are retained, so that we can keep track of the original multi-part geometry properties that each part came from:

```{python}
dat.explode()
```

The opposite transformation, i.e., "single-part to multi-part", can be acheived using the `.dissolve` method (@sec-geometry-unions).
