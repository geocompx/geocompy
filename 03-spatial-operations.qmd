...
if not file_path.exists():
  ...
  r = requests.get(
    "https://github.com/geocompx/geocompy/releases/download/0.1/landsat.tif",
    timeout=10,
  )
...
```{python}
points.apply(poly2.intersects).to_numpy()
```
:::

The `.intersects` method returns `True` even in cases where the features just touch: intersects is a 'batch-all' topological operation which identifies many types of spatial relation, as illustrated in @fig-spatial-relations.
...
```{python}
m = cycle_hire.geometry.apply(cycle_hire_osm.geometry.intersects)
m.to_numpy().any()
```
...
selection = grid.intersects(shapely.box(*nz.total_bounds))
grid = grid[selection]
