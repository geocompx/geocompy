...
Spatial join (`gpd.sjoin`) along with buffered geometries can be used to do that, as demonstrated below using a threshold distance of 20 $m$.
...
z = gpd.sjoin(cycle_hire_buffers, cycle_hire_osm.to_crs(crs))
z
```
...
The previous chapter (and especially @sec-manipulating-raster-objects) demonstrated how to retrieve values associated with specific cell IDs, or row and column combinations, from a raster.
...
To accommodate the results, we can, for instance, transform `elev` to the standard `int64` data type using `astype` before applying the operator.
