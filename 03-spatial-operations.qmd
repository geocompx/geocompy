...
Joining two non-spatial datasets relies on a shared 'key' variable, as described in @sec-vector-attribute-joining.
...
The starting point is to create points that are randomly scattered over the Earth's surface (@fig-spatial-join (a)):
...
The default is to omit them, which is usually not what we want.
With `{'color':'none','edgecolor':'black'}`, those polygons are shown with black outline and no fill.
...
(The `.distance` method syntax is thus analogous to the topological evaluations methods, such as `.intersects` (@sec-topological-relations), only that the results are numeric rather than boolean `Series`.
...
To illustrate, let's now take two regions in `nz`, Otago and Canterbury, represented by the object `co`:
...
In plain language, we take the geometry from each each row in `nz_height.iloc[:3,:]`, and apply the `.distance` method on `co` with that row as the argument:
...
This demonstrates the fact that distances between points and polygons refer to the distance to any part of the polygon: the second and third points in `nz_height` are in Otago, which can be verified by plotting them (@fig-nz-height-and-otago):
...
and create a corresponding random boolean mask named `mask`:
...
Note that some functions and operators automatically change the data type to accommodate the resulting values, while other operators do not, potentially resulting in overflow (i.e., incorrect values for results beyond the data type range, such as `255` for `int8`).
...
To accommodate the results, we can, for instance, transform `elev` to the standard `int64` data type before applying the operator.
...
Special care should be given to the edge pixels.
How should they be calculated?
The **scipy.ndimage** filtering functions give several options through the `mode` parameter (see the documentation of any filtering function, such as [scipy.ndimage.median_filter](https://docs.scipy.org/doc/scipy/reference/generated/scipy.ndimage.median_filter.html), for the definition of each mode):
...
(Another example, of the `gdal_contour` program, will be shown in @sec-raster-to-contours.)
...
@fig-raster-slope shows the result, using our more familiar plotting methods from **rasterio**.
The code section is relatively long due to the workaround to create a color key (see @sec-plot-symbology) and removing "No Data" flag values from the arrays so that the color key does not include them:
